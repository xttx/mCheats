...
namespace ScreenshotInterface
{
    ...
    public static class ScreenshotManager
    {
        ...
        static Dictionary<Int32, byte[]> _UpdateGraphicsRequestByClientPID = new Dictionary<int, byte[]>();

        public static void AddGraphicsUpdateRequest(Int32 clientPID, byte[] sendedImage)
        {
            try
            {
                if (!HookManager.HookedProcesses.Contains(clientPID))
                {
                    throw new ArgumentException("UG: The client process must be hooked first", "clientPID");
                }
                
                if (sendedImage == null)
                {
                    throw new ArgumentNullException("UG: updategraphics");
                }
                
                lock (_UpdateGraphicsRequestByClientPID)
                {
                    _UpdateGraphicsRequestByClientPID[clientPID] = sendedImage;
                }
            }
            catch
            {
        ...
        public static byte[] GetUGRequest(Int32 clientPID)
        {
            try
            {
                lock (_UpdateGraphicsRequestByClientPID)
                {
                    ...
                    _UpdateGraphicsRequestByClientPID.TryGetValue(clientPID, out result);
                    _UpdateGraphicsRequestByClientPID.Remove(clientPID);
